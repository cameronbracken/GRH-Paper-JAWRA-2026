...
The quality of hydrologic forecasts and hydropower scheduling has rapidly improved in recent years due to the emergence of new data driven approaches \citep{kratzertLearningUniversalRegional2019a} and scheduling optimization \citep{zhangOptimizationShorttermHydropower2024}. Despite this, the practice of forecast verification has remained mostly unchanged. A traditional forecast verification requires evaluation of hindcasts using standard statistical metrics, over a period which is representative of the full climotological record, often 30 years or more. The high cost of hindcasts and strict data requirements are two reasons full verifications are not done in practice. The second best approach, if hindcasts are not available, is to evaluate archived operational forecasts, although this approach fits poorly with products that are updated continuously, trained on evolving data streams, hand adjusted by operators, or developed by third parties. The gap this study addresses is to develop procedure by which hydropower or reservoir operators can evaluate forecasts on their own system under familiar operating conditions.
...
A general real-time forecast evaluation process can be described in the following steps. Steps 1 through 3 are generic practice, common to any verification exercise. Steps 4 and 5 are where this framework departs from retrospective verification.
\begin{enumerate}
...
where $\text{KGE}_{f}$ is the metric for the forecast being evaluated, $\text{KGE}_{ref}$ is the same metric computed for the reference forecast, and $\text{KGE}_{SS}$ is the KGE skill score. A skill score has a maximum value of 1 when the forecast is perfect, is 0 when the forecast performs no better than the reference, and is negative when it performs worse. The reference forecast is often taken to be the persistence forecast, the climatology, or some other existing forecast. Note that the NSE itself is a skill score where the reference forecast is taken to be the mean of the observed data.
...
In addition to the generic forecast evaluation metrics described above, we developed several tailored metrics specifically relevant to the GRH system and its operations. Each arose from the feedback process described in step 4 of Section \ref{sec:realtime}, and Table \ref{tab:tailored} traces each metric to the operator concern that prompted it. The first focused on day ahead forecast performance as shown in the previous section. This was motivated by the day ahead market that GRH bids into, as such they focus their attention heavily on the 1-2 day ahead period where the forecasts matter the most. Operationally, they adjust their flows daily, so no forecast is used after the 2 day mark.
