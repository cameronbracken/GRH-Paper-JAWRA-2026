...
added a smoothing sensitivity analysis, characterized the hydrologic characteristics of the evaluation
...
Two statements in the original manuscript have been corrected:
...
representative period, which expensive and may be infeasible. The fallback of evaluating
...
(comment 4.1), together with a sensitivity analysis; The proprietary data is stated 
as a limitation.
...
You are correct, persistence is often hard to beat at short lead times. We have stated
that persistence is competitive to about two days and then plotted it out to ten. Reviewer 2 raised a
...
nearby USGS gages, these reflect older operations and so would not reflect a reasonable climotology. 
...
We cannot provide calibration and validation results. The data used in this study was provided on
the basis that be be anonymized. The lack of detailed information about the models reflects the 
reality that Utilities face when procuring commercial forecasts. We have also added that a full procurement evaluation should weigh ongoing operational costs, data dependencies, 
and ease of integration alongside skill, since these
may differ substantially commercial products.
...
We have also added text describing the cascading system. Because releases from each dam become inflow to the next, an error in an upstream forecast propagates to the downstream forecast and compounds with lead
time.
